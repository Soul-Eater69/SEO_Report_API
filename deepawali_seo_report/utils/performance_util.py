...
class PerformanceUtil:
    ...
    def get_compression_data(self):
        ...
        resource_sizes = {
            ...
            "javascript": {"original": 0, "compressed": 0, "rate": 0},
            "image": {"original": 0, "compressed": 0, "rate": 0},
    ...
    def get_js_errors(self):
        ...
        driver = webdriver.Chrome(executable_path="D:\\driver\\chromedriver.exe",options=options)

        # Navigate to the website
        driver.get(self.url)

        error_strings = ["SyntaxError", "EvalError", "ReferenceError", "RangeError", "TypeError", "URIError"]
        js_errors = driver.get_log("browser")
        js_errors = [error for error in js_errors if any(error_string in error["message"] for error_string in error_strings)]
    ...
    def check_minified_files(self):
        ...
        result = {
            "css":css_minification_audit['score'] == 0,
            "javascript":js_minification_audit['score'] == 0
        }
    ...
    def check_depricated_html(self):
        ...
        if len(obsolete_tags) == 0:
            return {"depricated-html":False}
        else:
            return {"depricated-html":True}
